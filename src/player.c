...
PlayerOption no_player() { return (PlayerOption){.some = false}; }
...
PlayerOption parse_player_option(const char *str) {
  if (strcmp(str, "User") == 0)
    return (PlayerOption){true, User};
  if (strcmp(str, "Opponent") == 0)
    return (PlayerOption){true, Opponent};
  return (PlayerOption){false};
}

Player random_player() { return (rand() % 2 == 0 ? User : Opponent); }
