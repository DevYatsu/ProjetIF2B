...
Tile tile_with_piece(const ChessPiece piece) {
  return (Tile){.some = true, .value = piece, .captured_by = no_player()};
}

Tile deserialize_tile(const char *piece_str, const char *player_str,
                      const char *captured_by_str, const bool from_user_input) {
  PieceKind kind;
  ...
  if (strcmp(piece, "King") == 0 || strcmp(piece, "Roi") == 0) {
    kind = King;
  } else if (strcmp(piece, "Queen") == 0 || strcmp(piece, "Reine") == 0 ||
             strcmp(piece, "Dame") == 0) {
    kind = Queen;
