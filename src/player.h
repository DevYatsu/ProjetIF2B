 ...
 */
typedef enum { User, Opponent } Player;

/**
 * @brief Représente une option de joueur, qui peut être soit présente (some =
 * true) avec un joueur spécifique, soit absente (some = false).
 *
 * Cette structure est utilisée pour indiquer si un joueur est sélectionné ou
 * non.
 */
typedef struct {
  bool some;
 ...
 *
 * Cette fonction retourne une `PlayerOption` avec `some` à true et le joueur
 * spécifié.
 *
 ...
 *
 * This function returns "User" for the `User` player and "AI" for the `AI`
 * player.
 *
 ...
 *
 * If the option is present (`some` is true), it returns the string
 * representation of the player. If the option is not present (`some` is false),
 * it returns "_".
 *
 ...
 *
 * This function checks the input string and returns a `PlayerOption` with
 * `some` set to true if the string matches "User" or "Opponent". If the string
 * does not match either, it returns an option with `some` set to false.
 *
 ...
 * identifiants de joueur : `User` ou `AI`. Elle suppose que `srand()` a déjà
 * été appelé quelque part avant, afin de garantir une génération aléatoire
 * correcte.
 *
